...
Version 1.3 Notes:
    This version accounts for long-running commands & introduces a GUI
    for CSV file selection.
"""
...
def UserSelect():
    ...
    Note:
        The CSV should reside in the same directory as this .py file, be
        named IP_Addresses.csv, and have only one column with only IPs.
    """
    ...
    if (IPSource == '1'):
        ...
        print('')
        #print('Note: The CSV should reside in the same directory as this .py file, be ')
        #print('named IP_Addresses.csv, and have only one column with only IPs.')
        #print('')
...
if __name__ == "__main__":
    ...
    # Ask what command is to be ran on remote hosts
    print('')
    commands = []
    ...
    print('\n' * 2)
    print('Please note that if "[" and "]" are detected in the same line, this script will')
    print('assume that the device is asking a question. The default value will be chosen.')
    print('If this is not acceptable then this script may not be suitable for the command.')
